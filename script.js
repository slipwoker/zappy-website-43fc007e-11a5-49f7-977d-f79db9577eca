    ...
    fields.forEach(function (field) {
      ...
      if (field.hasAttribute('required') && value === '') {
        ...
        if (type === 'email' && !isValidEmail(value)) {
          ...
          if (value.length < min) {
            errorMsg = getFieldLabel(field, form) + ' must be at least ' + min + ' characters.';
          }
        } else if (field.hasAttribute('maxlength')) {
          const max = parseInt(field.getAttribute('maxlength'), 10);
          if (value.length > max) {
            errorMsg = getFieldLabel(field, form) + ' must not exceed ' + max + ' characters.';
          }
        }
      }
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '43fc007e-11a5-49f7-977d-f79db9577eca',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '43fc007e-11a5-49f7-977d-f79db9577eca';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '43fc007e-11a5-49f7-977d-f79db9577eca';
